...
Two executors have their own role docs that pre-define how they operate. Briefs inherit those workflow defaults instead of duplicating them. For engineering blocks, cite the authoritative plan/CP sections and include the task-specific objective, constraints, and acceptance subset needed for execution; do not create a competing rewrite of the plan.
...
- **Engineering-Lead** follows the repo-root `engineering-role.md` — read the ratified capstone doc from the repo, verify actual repo/environment state before building, plan then execute, delegate bounded tasks to subagents, own the debugging loop, and state plainly when a block clears or fails a capstone CP item. B-Claude briefs specify milestone, project state, and goal; they identify applicable architectural constraints and cite the authoritative plan/checkpoint criteria, carrying only the task-specific outcome subset — nothing about engineering workflow and no competing criterion set.
