...
- **Destructive local operations are owner-only too.** History rewriting, `git clean`, hard resets, and worktree removal outside the lifecycle above require an explicit instruction naming the operation.
...
- **Reclamation closes the checkpoint.** A checkpoint is not closed until `git branch -vv` shows `main` alone. The Orchestrator runbook is in `orchestrator-role.md`; the Landing Report that feeds it is `docs/track-b/gauntlet-templates.md` §9.
