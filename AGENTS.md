...
- **Every closed checkpoint gets exactly one disposition for its own `gauntlet/<checkpoint>` branch.**
  **LAND** — Yarden runs `git merge --squash gauntlet/<cp>`, reviews the staged tree, and commits **by hand**; then **two tags, because there are two things worth marking**: `git tag land/<cp> <the squash commit on main>` records where the work landed, and `git tag evidence/<cp> <evidence_tip_sha>` preserves the reviewed candidate chain. **The second is the one the tag-before-delete rule requires** — a squash commit does not contain the candidate SHAs the verdicts cite, so tagging only the landing point leaves the chain reachable from nothing once the branch is reclaimed. `git merge --squash` deliberately stages without committing, which is the whole point: the commit that reaches `main` is authored, not generated. It also collapses candidate, component verdicts and evidence tip into one commit, so the two-terminal-SHA ordering never propagates to `main`.
