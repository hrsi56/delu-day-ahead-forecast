"""CP-20 research scores: CP-16's frozen scoring with the single HG-H0 contrast.

Adapted from ``src/cp16/scoring.py`` (preserved unchanged): identical metrics, WIS weights,
equal-fold B0 normalisation, per-fold/hour/block/peak/recovery diagnostics, support rule,
original section 8 criteria and the seed-15042, 2,000-replicate, 7-calendar-day paired
block bootstrap (same generator, same index set). Changes are confined to the arm names,
the sole primary contrast HG-H0, the H0-on-exact-ties descriptive order, the
arm-appropriate parity check (identical keys/truth/price-only A1 scale; centrals differ by
construction) and fallback-incidence reporting. No fitting, search or promotion.
"""
...
def validate_predictions(predictions, expected_keys, *, production=True):
    ...
    h0 = frame.loc[frame.policy.eq("H0")].set_index(KEYS).sort_index()
    hg = frame.loc[frame.policy.eq("HG")].set_index(KEYS).sort_index()
    if not h0[["scale", "level", "y_true"]].equals(hg[["scale", "level", "y_true"]]):
        raise ValueError("H0/HG price-only A1 scale, level and truth parity failed")
    if production:
...
def _selection(metrics, diagnostics, *, integrity_verified=True):
    ...
    ranking = sorted(CANDIDATES, key=lambda p: (scores[p]["S_WIS"], scores[p]["S_MAE"], p != "H0"))
...
def _evaluate(predictions, expected_keys, *, production, replicates, lineage=None):
    ...
    summary = dict(ranking=ranking, ranking_rule="S_WIS, S_MAE, H0 on exact ties; descriptive only, not promotion",
                   ...
                   primary_contrast="HG-H0", secondary_contrasts=[],
                   joint_rule="observed joint improvement only if upper CI(dS_WIS)<0 AND upper CI(dS_MAE)<=0",
                   ...
                   economics="historical economics descriptive only; zero new economic runs or thresholds")
    fallback = _fallback(lineage) if lineage is not None else pd.DataFrame()
    return dict(metrics=metrics, diagnostics=diagnostics, uncertainty=uncertainty, criteria=criteria,
                fallback=fallback, summary=summary)


def _fallback(lineage):
...
def evaluate(predictions: pd.DataFrame, expected_keys: pd.DataFrame, lineage: dict | None = None) -> dict:
    """Evaluate exactly 75,229 production rows using the frozen 2,000 draws."""
    return _evaluate(predictions, expected_keys, production=True, replicates=BOOTSTRAP_REPLICATES, lineage=lineage)
