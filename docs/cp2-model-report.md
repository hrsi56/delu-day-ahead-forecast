...
- Code SHA at the final run: `525fc2d4c5bc8763b0da30197030740c49c73b34` — the commit whose tree produced these
...
- Frozen champion fingerprint: `57e3ad40a7f48bb7896628ce2e5dec61314dea567da9867aede4bf1b0a10e0fb`
...
metric, the augmented arm is *lower* on 2 of the 5 folds:

| fold | base | base_plus_residual_load_proxy | pct_vs_base | lower_arm |
|---|---|---|---|---|
| fold_1 | 2.0705 | 2.0988 | 1.3633 | base |
| fold_2 | 5.9295 | 6.3029 | 6.2978 | base |
| fold_3 | 45.8522 | 45.7021 | -0.3274 | base_plus_residual_load_proxy |
| fold_4 | 5.3795 | 5.4413 | 1.1488 | base |
| fold_5 | 6.5620 | 6.4859 | -1.1588 | base_plus_residual_load_proxy |
...
similar-day naive on mean pinball loss in **5 of 5 folds** and on median MAE in
...
measured rather than asserted: 5/5 and 10/10
overlap at the top, Spearman rank correlation 0.984 across all features.
...
Measured wall clock on arm64 / Darwin 25.5.0, CPU only,
Python 3.13.15, from `reports/cp2/timings.json`:

| Stage | Seconds |
|---|---|
| Development (baselines + both catalogs, 5 folds) | 214.9 |
| §7.2 post-gate benchmark | 208.1 |
| Final fit, freeze, sequential proof, holdout | 204.0 |
| Diagnostics (SHAP, permutation, regime, reliability) | 68.4 |
| **Total** | **695.4** |
