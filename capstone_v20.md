...
| **CP-13** | **The live scorecard** (§7) | CP-12 | Page re-rendered and committed by the job; `docs/index.html` still makes **zero runtime calls**, re-scanned independently; **no date filter**; `make verify` **and** `uv run python scripts/check_links.py` both green — they cover different things and neither substitutes for the other: `verify` binds the item-5 *claim* set (cutoffs, catalog, metrics, evidence class, benchmark) across surfaces, while link resolution and URL correctness are the test suite's and `check_links`' job; the `live_` namespace guard shipped with its positive control; every number carries its source model; the staleness banner proved by a simulated missed run |
