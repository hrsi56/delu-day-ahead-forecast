# Program Stage Sequence — Full Ordered Map (v5)

*Rebuilt 2026-07-22 by the orchestrator on Yarden's explicit request (the v6.3 strict-gate amendment ratification authorized the rebuild — the sanctioned path per ORC §"The Program Stage Sequence map"). **Ratified edition** — derived from the current Strategic Anchors: `syllabus_v3_2.md` v3.2, `capstone_V6_3.md` v6.3, and the companion `Binary Classification Mini-Capstone.md` v1.0. Supersedes v4 (2026-07-15). Static by design: no status markers; all position tracking lives in progress.md. On any conflict between this map and the anchors, **the anchors win.***

**v4 → v5 delta (strict-gate + audit alignment, 2026-07-22 — a genuine sequence/row change).** Every v4 row carries forward (omission audit below); the changes are: (1) **SQL row split** — **SQL-A** (a minimal production subset, authored at **M1 before CP-1**, integrated as the capstone's DuckDB feature/data-quality mart) and **SQL-B** (interview-fluency remainder through **G3**); the 20h cap is preserved and split; the toy Postgres/Supabase table is replaced by **DuckDB** (capstone §9.6). (2) **Funnel reconciliation** — post-M2 Track C is outreach / prep / pipeline-building; SQL-screen-exposing active submissions gate on SQL-B completion (G3). (3) **C8 row** gains the **SQL / A-B / cloud / LLM requirement-ratio** capture (feeds DEC-ALG at G3 **and** DEC-AWS at G5). (4) **M1** carries the strict-gate PIT proof + gate-feasible candidates + SQL-A mart; **M3** gains the **§7.2 post-gate information benchmark**; **M4** gains the **offline data/output-health report** (capstone §9.5). (5) **All engineer-workflow routing repointed `engineering-role.md` → `engineering-role.md`** — engineering-role.md is now the hierarchy/bootstrap doc, not the engineering role. (6) **Anchors bumped** to capstone v6.3 / syllabus v3.2. (7) **DEC-AWS cascade updated:** a "yes" now amends capstone **v6.3 → v6.4** and rebuilds this map **v5 → v6**. No math-arc, month-structure, or companion-arc change.

**Omission audit (v4 → v5).** Every v4 row is present with its ID and order; the only sequence change is the SQL split (one v4 row → SQL-A + SQL-B in their correct positions). No row dropped.
...
**Verification status.** Rows for Months 0–5 (setup → G5) were audited row-by-row against the v4 anchors on 2026-07-15 and **re-aligned to `syllabus_v3_2.md` v3.2 / `capstone_V6_3.md` v6.3 at v5 (2026-07-22)** — the strict-gate changes touched the M1–M5 / CP-1–CP-4 rows and the SQL / C8 / funnel rows; every v4 row is preserved. **The FM/FCP/G6 rows were NOT audited against `Binary Classification Mini-Capstone.md`** — that plan is stage-gated (ORC: not read, briefed, or consumed before B-Man3/FM0). They are carried forward **verbatim from v3**, where they were authored at the 2026-07-12 ratification. They are marked ⚠ below and are due one verification pass when the gate opens at B-Man3.
...
- **Claude Code rows:** brief fields per ORC §"Type B-Claude"; workflow per that repo's `engineering-role.md`; the brief points the engineer at the plan doc for architecture and acceptance criteria, never restates them. **Every such brief names its repo** (column below). FM rows point at the **fraud repo's own** `engineering-role.md` + FRD once that repo exists (FM0 creates it).
...
1. **Flagship** — `hrsi56/delu-day-ahead-forecast` (holds `engineering-role.md` + `capstone_V6_3.md`). All M/SPEC rows.
...
| C8 | *(parallel, Months 0–1)* Target-list research: tiered Beer-Sheva-local + ≤3-day-hybrid TLV/Herzliya DS employer list. **Captures the take-home-vs-live-screen ratio (feeds DEC-ALG at G3) and (v5) the SQL / A-B / cloud / LLM requirement ratios (feed DEC-AWS at G5)** | Research agent | PRG Track C "C8-research" line · SYL:Supp "ALG" (downgrade rule) · PRG "DEC-AWS" inputs |
...
| M1 | **Capstone data layer:** ENTSO-E + SMARD bulk pull, UTC/DST/15-min handling incl. chunk-boundary bin integrity, feature catalog v1 (**gate-feasible candidates** + train-only climatology on the proxy series + regime indicators), **strict-gate PIT proof — A69 excluded from champion (§5.2)**, **DuckDB SQL-A mart (§9.6)**, **champion/benchmark schema-separation invariant (§9.4)**, KFT/LAG leakage audit, A65/A01 ~25h head gap, **R-2 strict-gate resolution applied**. **Repo: flagship.** Verify actual repo/branch state before building | Claude Code | CAP:§12 build-routing note (incl. the v6.3 additions) + "M1" + **§0** + **§1** + §3, §4.0–4.1, §5.1–5.2, **§7.2, §9.3, §9.4, §9.6** + **§11** (R-2 strict-gate; R-3; R-4) · PRG Notes |
...
| CP-1 | Capstone checkpoint 1 (expanded at v6.2/v6.3): full checklist incl. the three §4.2 spectral items — `spectral_eda.py` end-to-end, ≥3 figures, peaks annotated, README ¶ — **+ (v6.3) strict-gate PIT availability, gate-feasible candidates, DuckDB SQL-A mart tested, champion/benchmark schema separation** | Orchestrator | CAP:§12 "CP-1" checklist (incl. the v6.2 + v6.3 items) |
| SQL-A | **(v5 — at M1, before CP-1)** Minimal production subset of window/data-quality queries hand-authored and integrated as the capstone's **DuckDB** feature/data-quality mart (capstone §9.6); tested against the §9.4 invariants. Part of the 20h SQL cap | Yarden solo (integration reviewed by Claude Code) | SYL:Supp "SQL" (SQL-A/B split) · CAP:§9.6 |
...
| M2 | **Capstone baselines + model + proxy selection:** similar-day naïve + naïve-168h + Ridge + LightGBM 9-head on pinned folds, DM vs. naïve (incl. secondary gate), MLflow → DagsHub, thin CI. **(v6.3) 2×2 matched-ablation proxy selection against the pre-registered retention rule; winning strict-gate catalog frozen at CP-2 before M3.** **Repo: flagship** | Claude Code | CAP:§12 "M2" + §4.1 (selection rule) + §6, §7, §7.1, §5.1, §9.1, §9.4 · SYL:M3-interleave |
| CP-2 | Capstone checkpoint 2: ≥5 public MLflow runs, CI green, DM secondary gate reported; **(v6.3) proxy selection complete + strict-gate catalog frozen ("no proxy survives" is valid); M2 calibration = raw-head only** | Orchestrator | CAP:§12 "CP-2" checklist |
...
| M3 | **Capstone diagnostics:** CQR on the pinned calibration slice, isotonic last, **SHAP explains the frozen champion (selection closed at M2/CP-2)**, permutation importance, **§7.2 post-gate information benchmark (five metrics)**, regime-stratified error table, **three-stage reliability diagrams (raw/post-CQR/final)**. **Repo: flagship** | Claude Code | CAP:§12 "M3" + §6.2, §7.2, §8.1–8.4, §4.1 + **§11 "R-1"** (name regime-shift failure, don't gloss) · Contingency: CAP:§13 EnbPI reopen (any stratum >10 pp) |
...
| CP-3 | Capstone checkpoint 3 (v6.3): coverage ±5pp on ≥4/5 folds (post-CQR stage); three-stage coverage (final empirical); moved CQR-monotonicity; SHAP top-10 reported (**no named-feature gate**); SHAP/permutation overlap **descriptive**; §7.2 benchmark reported; stratified table documented | Orchestrator | CAP:§12 "CP-3" checklist |
...
| M4 | **Capstone local showcase:** marimo end-to-end, champion registered `Production`, **retained-feature** slider lookup + per-cell OOD flags, **incl. the §10 item-(3b) spectral section (static, NO slider)**, **+ (v6.3) the offline data/output-health report generated (§9.5, ≤~8 h)**. **Repo: flagship** | Claude Code | CAP:§12 "M4" + §9.2, §9.5, §10 item 3b, §6.1 |
| CP-4 | Capstone checkpoint 4: registry tag live, <10s local cold render, **retained-feature sliders** from lookup, OOD flags validated, **(v6.3) offline health report generated** | Orchestrator | CAP:§12 "CP-4" checklist |
...
| DEC-AWS | **Pre-committed AWS-backbone decision at G5:** adjudicate parked decisions D1–D6 + D8 against the C8 target-list JD cloud-requirement ratio, live funnel/interview signal, and February capacity. A "yes" amends capstone **v6.3 → v6.4** and rebuilds this map **v5 → v6**; until then no AWS arc enters the ratified sequence | Orchestrator | CAP:§1 delta paragraph · AWS:§12 · C8 output · PRG Track C funnel/capacity state |
...
Audited 2026-07-15 against v3.1 / v6.2; **re-aligned 2026-07-22 to `syllabus_v3_2.md` v3.2 and `capstone_V6_3.md` v6.3** (v5 strict-gate — M1–M5 / CP, SQL, C8, funnel rows updated; every v4 row preserved).
