...
4. The CP-1 candidate `368822de…` did not land. **Superseded by events, 2026-09-14:** the v6.8 remediation was built on the same branch and landed at `5651c9a` (squash, one parent); `gauntlet/cp-1` was then reclaimed. **The citation follows the ref — read the chain at the tag `evidence/cp-1` → `24f59b5…`, not at the deleted branch.** Verified after deletion: `368822de…`, `cd9b6dd…`, `c0b6e180…` and `8adddcb…` are all reachable from that tag, and both the superseded and the fresh Integration verdicts are readable through it. **The independent pre-landing audit and the two reviews of this amendment are not in the history** — the Owner chose to hold them externally, and no tag preserves what was never committed. *(Originally corrected 2026-09-09 after an independent review found this record claiming the audit was branch-reachable.)*
