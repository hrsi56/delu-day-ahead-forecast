...
def test_the_shipped_fixture_is_the_committed_one():
    """The answer key is committed and reviewed in git, never regenerated by the
    build it judges; the payload builder refuses to proceed if the frozen champion
    no longer reproduces it."""
    committed = (REPO_ROOT / "tests" / "fixtures" / "wasm_equivalence_fixture.json").read_text()
    assert (PUBLIC / "fixture.json").read_text() == committed


def test_the_fixture_exercises_every_calendar_branch(payload):
    """Round 1 broke the bridge-day logic and this gate still passed. Not again."""
    ...
    seen = {name: set() for name in ("is_federal_holiday", "is_day_after_holiday", "is_bridge_day", "day_type", "dst_transition_day")}
    for entry in fixture["days"]:
        matrix, _ = champion.features_for_day(entry["day"])
        for name in seen:
            seen[name] |= {float(v) for v in matrix[:, columns.index(name)]}
    assert seen["is_federal_holiday"] == {0.0, 1.0}
    assert seen["is_day_after_holiday"] == {0.0, 1.0}
    assert seen["is_bridge_day"] == {0.0, 1.0}
    assert seen["day_type"] == {0.0, 1.0, 2.0}
    assert seen["dst_transition_day"] == {0.0, 1.0}
    rows = {entry["rows"] for entry in fixture["days"]}
    assert {23, 24, 25} <= rows, f"both DST directions must be present, got row counts {rows}"
