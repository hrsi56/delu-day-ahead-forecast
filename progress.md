...
*Living document, maintained under `orchestrator-role.md`. **Regenerated 2026-09-08 on CP-1 dispatch.** Anchors: `capstone_V6_7.md` **v6.7** (ratified 2026-09-07); `program-stage-sequence.md` **v8**. `syllabus_v3_2.md` is an optional resource and is not an anchor. `main` = `origin/main` = **`7aaa02b`**, working tree clean — the v6.7 application, its merge repair, and the `AGENTS.md` Q&A-capture rule are all landed and published.*
...
- **Anchor:** **`capstone_V6_7.md` v6.7**, owner-ratified 2026-09-07. Amendment record: `docs/track-b/capstone_V6_6-to-V6_7-amendments.md`, whose **post-ratification repair addendum (2026-09-07)** supersedes its *Application scope* bullet and acceptance items 1, 12 and 15. Read the addendum before re-running that acceptance test.
- **Position: M0 DONE. M0.5/CP-0 CLOSED and RETIRED. M1/CP-1 DISPATCHED 2026-09-08.** The brief is written and carried to the Engineering Lead. Ten checklist items, one fresh Integration Critic, **approximately 10 hours**.
- **CP-1 runs on the SMARD route, and that is inside the plan.** ENTSO-E's Web API is down (below), so the brief directs the bulk pull through SMARD under §3's **"fallback-primary if ENTSO-E access degrades"** clause. This is an execution decision recorded here, **not an amendment** — the contingency is ratified text.
- **CP-1's expected terminal status is `INCOMPLETE`, and that is a correct outcome.** Item 5 is the ENTSO-E↔SMARD price reconciliation and cannot be evidenced from one source. Nine items are expected to close; item 5 waits for ENTSO-E and closes in a short follow-up. The brief forbids substituting a different comparison or weakening any other item.
...
- **What exists in the tree today:** the Month-0 spike only — `src/spike/entsoe_helpers.py`, `scripts/q1_*.py … q8_*.py`. **No product code, no tests (`tests/` is empty), no CI.** §9.4's eight property tests and the thin CI are CP-1 scope to be written, and the plan says so explicitly.
...
- **Governance Lockdown in force** since 2026-08-08 (`f45e577`). Five task-scoped suspensions have been granted and spent: the v6.7 application, three 2026-09-07 repair rounds, the 2026-09-08 merge repair, and the 2026-09-08 `AGENTS.md` Q&A-capture section. None granted publication or unrelated authority.
...
- **Interview prep has started, ahead of activation.** `שאלות תשובות.docx` is tracked in the repository and carries two entries — the ENTSO-E outage and SMARD fallback, and the 2019 window start.

---

## Setup State

- **Project Knowledge swap — owner action pending:** `capstone_V6_7.md`, `program-stage-sequence.md` v8, this file, and the current role/router docs.
...
- **OPEN — ENTSO-E's External API is disabled with no ETA.** Blocks PRE-1 and CP-1 item 5. **No owner or agent action exists** — it is a wait on a third party. It does **not** block CP-1 itself, which proceeds on SMARD.
...
- **The next Track B event is CP-1's terminal return.** Read it for protocol defects as well as its verdict. Expect `INCOMPLETE` with item 5 open; that is correct and is not a re-brief trigger.
