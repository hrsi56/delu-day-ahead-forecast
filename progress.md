...
- **M1 brief (syllabus Month 2):** inherits the **v6.2 §12 binding list** (incl. the **§4.2 spectral-EDA block** — module + 3 figures + README ¶ + R-5) **and the four sections the v3 map omitted: §0, §1, §9.4, §11** + M1 cosmetic (verify Dec-2024 / Jan-2025 Dunkelflaute peaks; tighten §2.2 sentence). **SPEC** briefable inside M1 or as a follow-on B-Claude block, closing before CP-1. **Verify at brief time:** the flagship `CLAUDE.md` referenced `capstone_V6_1.md` **by filename** (map v4 audit-trail conflict #5) — with v6.2 now on `main`, confirm the pointer resolves, or the M1 brief must name the plan doc explicitly. Claude Code verifies repo state at block start regardless.
