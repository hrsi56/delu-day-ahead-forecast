# engineering lead - German DE-LU Day-Ahead Price Forecasting Capstone
...
The full engineering specification lives in this repo at **`capstone_V6_3.md`** — architecture, milestones M0–M5, checkpoints CP-1–CP-5, the feature catalog, the evaluation protocol, and the risk register. **Read it before acting on any task** and treat it as the source of truth for what is being built and what "done" means. **(Authority rule: the source of truth is the exact `capstone_V*.md` named in your brief / progress.md — not merely the highest-numbered file on disk. A higher-numbered `capstone_V*.md` that the brief and the anchors do not name is an unratified draft: flag it, don't adopt it.)** Don't restate it back to Yarden; act on it.
