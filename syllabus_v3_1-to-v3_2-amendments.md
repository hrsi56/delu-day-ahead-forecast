...
*Authored 2026-07-22 by the orchestrator. Aligns the syllabus with **`capstone_V6_3.md` v6.3** (the strict-gate methodology amendment) and repairs two classes of stale content that predate it. The changes are **already applied** in `syllabus_v3_2.md`; this sheet is the change record for the amendment chain. No math-arc content, month structure, depth-label vocabulary, or historical delta entry was deleted; the time budget is unchanged beyond the net-zero SQL-A/SQL-B split. All prior rejected-item records (LO3, LO2, SC3-learn, SC5-learn) carry forward unchanged.*
...
- **Month-1 feature-catalog line:** gate-feasible candidates (`gate_feasible_residual_load_proxy`, `expected_scarcity_proxy`) decided by M2/CP-2 matched ablation (no pre-ratified headline); A69 excluded from the champion, confined to the §7.2 benchmark; the DuckDB SQL-A mart as canonical source.
...
Post-M2 Track C activity is **outreach / prep / pipeline-building**; **SQL-screen-exposing active submissions gate on SQL-B completion (G3)**; the start-of-Month-5 backstop is unchanged.
