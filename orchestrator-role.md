...
3. **Phase-trigger (continuous activation).** After capstone M2 lands (similar-day-naïve + Ridge baselines + LightGBM quantile ensemble + Diebold-Mariano test all shipped, MLflow showing ≥5 experiments) — projected in syllabus Month 3. From M2 forward, Track C becomes continuously active: applying, recruiter outreach, interview prep, target-company research. Active applications begin no later than the start of syllabus Month 5. This is the application phase; rules 1, 2, and 4 no longer apply, and Track C is first-class. From Month 6 the application funnel runs on both artifacts.
...
- **Acceptance criteria** — outcome-level only (commit hash, passing test, working endpoint, decision documented), mapped to the relevant CP/FCP checklist items where the block feeds a checkpoint so Claude Code can report checkpoint status. Never at the implementation-step level.
...
Complete the program — the ratified syllabus and both capstone artifacts — with Yarden **actively applying from the start of program-Month 5** at a target of **NIS 35K** for an industry Data Scientist role. Under the v3.1 expansion (an explicit owner decision to optimize hire probability over the calendar envelope): the flagship envelope closes at G5 ~mid/late-January 2027 with the deployed DE-LU showcase live; the companion fraud mini-capstone runs in Months 6–7 inside the active application phase and closes at G6 ~March 2027. These dates are the program envelope, **not** a guaranteed signed offer. Applications do not wait for program completion: Track C's phase-trigger fires after flagship M2 (Month 3) and active applications begin no later than the start of Month 5, so Yarden is in-market with a strengthening portfolio well before either envelope closes. The DE-LU day-ahead forecasting tool positions him as a quantitative forecaster with honest uncertainty communication — regime-aware methodology backed by a published clinical-research statistical track record; the companion classification artifact adds the decisioning counterpart (class imbalance, cost-based thresholds, calibration) aligned with the fraud/cyber weighting of the Beer-Sheva market. Track pace in progress.md.
