...
- **Rebuild only on explicit request.** Regenerating the map is a block-level task Yarden explicitly asks for (e.g., after a re-ratification changes the sequence) — never routine maintenance, never part of a session close. **DEC-AWS exception already encoded in its ballot:** approving D8 at G5 is itself the explicit authorization to rebuild the map from its current v7 to v9 (v8 is deliberately unused, keeping plan and map numbering aligned); do not ask for a second map go/no-go.
