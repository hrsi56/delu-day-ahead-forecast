...
You do not audit evidence files yourself, and you have no shell: the record-level provenance rules — run/support roots, integrity manifests, candidate SHA/tree binding, computed verdict staleness over each review's declared `reviewed_paths`, plan identity and excerpt proof, input and evidence hashes, commands and exit codes, evidence-ref verification — are mechanically enforced by `scripts/gauntlet_protocol.py validate-verdict` and `verify-ref`. **The packet must report, for every Critic run, the exact command invoked and its exit code, and a terminal `validate-verdict` and `verify-ref` pass over every relied-on record and cited ref.** Any nonzero exit, missing run, or unreported record invalidates that verdict and therefore `PASS`. Your gate is: the full checklist is mapped to evidence; every mandatory surface and oracle has its own independent verdict; the reported validator runs cover every relied-on record and cited ref and all passed; and a fresh Integration-Critic `PASS` exists at the same final candidate SHA/tree. A non-`PASS` packet may record `NOT_RUN` with the exact reason and evidence.

The packet must also name the frozen evidence root produced by `freeze-evidence` at terminal return; live `.gauntlet/` paths alone are not durable evidence. Inspect the packet and its reported validator results, not the internal workbench. If the packet supports `PASS`, record the closure in progress.md, summarize the evidence, and explicitly ask Yarden whether to authorize the next stage. Do not pre-plan, brief, or start that stage until his go/no-go plus scheduling direction arrives. `BLOCKED`, `PLATEAU`, or `BUDGET_EXHAUSTED` never advance the track; they require an Orchestrator decision on remediation or a replacement ceiling. A reduced bar requires a prior owner-ratified capstone/checkpoint amendment and a new plan anchor; it cannot be improvised in a remediation brief.

**CP-2 additional receipt gate.** When the closing checkpoint is CP-2, the mandatory label-blind four-catalog chain also applies; its receipt gate is in `docs/track-b/cp2-blind-protocol.md` §4. Read it only then.
