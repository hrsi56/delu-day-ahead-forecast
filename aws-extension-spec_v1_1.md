...
- **D5 — Monitoring is custom-built, operationalized (not net-new), and manually triggered.** v6.3 already ships an **offline batch data/output-health report** (capstone §9.5); the AWS arc **operationalizes it as a SageMaker Processing step inside the manually-triggered `cloud-refresh` pipeline** — **not a schedule** (no EventBridge, no wake-ups, per D3) — publishing custom CloudWatch metrics + alarms. Not SageMaker Model Monitor, which **closed to new customers on 2026-07-30** and is unavailable to an account onboarding in 2027. A deliberate, defended design choice (§9, Q2), not a workaround.
...
| Monitoring | **Offline batch data/output-health report** (capstone v6.3 §9.5) | **Same report operationalized as a Processing step in the manual pipeline**: PSI on the champion's retained features + price distribution vs. training baseline, data-quality counts (nulls, bin integrity), forecast sanity (quantile monotonicity) → CloudWatch metrics + alarm → email (no schedule) |
...
**Recommended (D8):** G5 closes ~mid/late-Jan 2027 exactly as ratified (the AWS arc does **not** touch the flagship's pre-G5 critical path — inserting it earlier would delay the deployed showcase, the worst possible trade). **Month 6 (≈ Feb–Mar):** M6 at a realistic ~10 h/wk alongside active applications → **CP-6 closes ~mid-March** (the §7 honest estimate; §8's earlier end-Feb figure is superseded, per the 2026-07-22 rebase). **Months 7–8 (≈ Apr–May):** companion FM0–FM5 as specified in its v1.0 doc, unchanged in content; **G6 → ~late-Apr/May 2027.** B-Man3 (Kaggle account) shifts from "Month 5" to the Month-6→7 seam; a new **B-Man-AWS** (account + IAM + Budgets, **no SageMaker resource**) lands at AWS-1 day 1. Program envelope ~680 h → **~740 h**.
