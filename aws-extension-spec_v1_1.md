...
**Status: DRAFT v1.1 (semantically rebased 2026-07-22) — PARKED as pre-committed decision point DEC-AWS, adjudicated at the Month-5 gate (G5).** On ratification this document amends `capstone_V6_3.md` → **v6.4** (additive amendment + one rewritten §13 boundary, same pattern as the v6.2 spectral amendment). **v1.0 → v1.1 delta (2026-07-20, orchestrator revision under the owner's parking decision):** (1) **D7 extracted and ratified separately** as the capstone v6.2 M5 rider — the static first-touch page now exists by ratified plan; this spec builds on it rather than proposing it, and the ratification package is **D1–D6 + D8**; (2) the CP-6 item-13 CV-slot conflict corrected — the AWS portfolio update routes as a **C-Man-P-style manual surface update consuming no CV-iteration slot** (the three-slot cadence is exhausted at CV-3/M5); (3) the M6 load estimate restated honestly for application-season pace, with a pre-authorized AWS-2 de-scope; (4) all baseline references rebased to the post-rider capstone, target its ratification edition; D7's hours removed from the envelope (**~50–70 h**); §8's CV-delta claim annotated as the object DEC-AWS tests. Authored 2026-07-19 against capstone v6.2 in full, the 2026-07 SageMaker cost/effort research memo, and progress.md; revised 2026-07-20. The Model Monitor closure (D5's factual basis) was independently verified against AWS documentation on 2026-07-20. All AWS prices are official mid-2026 list rates from that memo unless flagged; a 10-minute re-verification of the pricing table is a mandatory first task of the arc (§10, R-8). **Semantic rebase (2026-07-22, under the v6.3 strict-gate ratification — this spec stays PARKED; the G5 DEC-AWS decision object is unchanged):** (1) **Version cascade** — the current capstone is now **v6.3** (strict-gate methodology amendment); this spec's baseline is **v6.3** and its ratification produces **v6.4**; the map cascade is **v5 → v6**. Inline `v6.2`/`v6.3` section-references below denote the same sections (v6.3 renumbered nothing), read against the current baseline. (2) **Monitoring is not net-new and not scheduled** — v6.3 already ships an **offline batch data/output-health report** (capstone §9.5); the AWS arc **operationalizes it inside the manually-triggered `cloud-refresh` pipeline**, never on a schedule (no EventBridge, no wake-ups — consistent with D3). (3) **Monitoring inherits capstone §9.5's feature-aware matched-reference rules** — PSI runs only on the champion's actually-retained non-deterministic features; continuous-feature references are reweighted to the comparison window's joint calendar-month × hour-of-week mix and use frozen training-quantile edges; continuous and categorical bin masses both receive **ε = 1e-6 smoothing + renormalization**; categorical/low-cardinality features use category-frequency PSI; and a continuous feature with <3 usable bins falls back to categorical only at ≤10 distinct training values, otherwise `insufficient data` amber. The same ≥90% completeness rule applies. The old A69-derived names (`residual_load_deviation_from_normal`, `scarcity_stress`, `dunkelflaute_flag`) are benchmark-only at v6.3 and are **not** monitored. (4) **Slider references follow the eventual CP-4 controls** (retained-feature sliders), not a pre-ratified trio. (5) **Internal-defect reconciliations:** the D8/§7 timing is unified on §7's honest estimate (**CP-6 ~mid-March/early-April, G6 ~late-May/early-June**), superseding §8's earlier end-Feb; **CP-6 item 9 observes the first full calendar month after the first billable resource is created**, which fits inside the 6–8-week build arc; and if the AWS-2 drift de-scope is taken, **CP-6 item 3's DAG drops `drift-metrics`** (it moves to the post-CP-6 increment with item 7). All prior v1.0 → v1.1 deltas stand.
...
- **D5 — Monitoring is custom-built, operationalized (not net-new), and manually triggered.** v6.3 already ships an **offline batch data/output-health report** (capstone §9.5); the AWS arc **operationalizes that same feature-aware, calendar-matched PSI and completeness logic as a SageMaker Processing step inside the manually-triggered `cloud-refresh` pipeline** — **not a schedule** (no EventBridge, no wake-ups, per D3) — publishing a bounded CloudWatch summary + alarms while retaining the full per-feature report artifact. Not SageMaker Model Monitor, which **closed to new customers on 2026-07-30** and is unavailable to an account onboarding in 2027. A deliberate, defended design choice (§9, Q2), not a workaround.
...
- **D8 — Sequencing.** M6 runs in **program Month 6 (≈ Feb–Mar 2027)**, immediately after G5. The companion arc shifts to **Months 7–8 (≈ Apr–May 2027)**; **G6 moves to ~late-May/early-June 2027**. Program hours: ~705 → **~755–775** (+50–70 h — D7's hours moved into ratified M5 scope). Full cascade in §8.
...
**"Work against the full capstone, in coordination with it."** §11 is a section-by-section amendment map against v6.3's actual text — every touched section, every preserved invariant, and the one §13 boundary this extension rewrites.
...
| Monitoring | **Offline batch data/output-health report** (capstone v6.3 §9.5) | **Same report operationalized as a Processing step in the manual pipeline**: feature-aware PSI on the champion's retained non-deterministic features against the frozen training reference matched to the window's calendar-month × hour-of-week mix (same completeness, binning, and categorical rules), data-quality counts, matched interval-width checks, and quantile sanity → bounded CloudWatch summary + alarm → email (no schedule) |
...
**4.8 Monitoring (custom; D5).** A Processing **step inside the manual `cloud-refresh` pipeline** (same container, ml.m5.large, ~3–5 min; **no schedule**), operationalizing the v6.3 offline health report (capstone §9.5). It computes feature-aware PSI only on **the strict-gate champion's actually-retained non-deterministic features**: the frozen training reference is reweighted to the comparison window's joint calendar-month × hour-of-week mix; continuous features use deduplicated frozen training-quantile edges; continuous and categorical bin masses receive **ε = 1e-6 smoothing and renormalization**; categorical/low-cardinality features use category-frequency PSI; and the deterministic <3-bin rule carries over (categorical fallback only at ≤10 distinct training values, otherwise `insufficient data` amber). The ≥90% completeness rule also carries over unchanged. The A69-derived `residual_load_deviation_from_normal` / `scarcity_stress` / `dunkelflaute_flag` remain benchmark-only and are not monitored. The same job emits data-quality counts (nulls, 4-of-4 quarter-hour bin integrity), matched interval-width checks, and forecast sanity (zero quantile crossings). The full per-feature result remains in the committed report artifact; CloudWatch receives a fixed, bounded summary metric set (for example maximum eligible-feature PSI, amber/red-feature count, data-quality failures, interval-width status, crossing count, and pipeline status) + **2 alarms** (health-threshold breach; pipeline failure) → SNS email, with the exact ≤10-metric set frozen before AWS-1 results are inspected. Cost **≈ $0.02–0.05/run + $0 CloudWatch**. *Evidence:* the health dashboard PNG, refreshed into the static page. Model Monitor / Clarify are unavailable to new accounts (closed 2026-07-30) and would not be chosen anyway — see interview Q2 (§9).
...
*Sections 4.1–4.9 above are the component spec; CP-6 references them as §5.x equivalents where the draft circulated — final v6.4 merge will renumber cleanly.*
...
- [ ] 7. **(Conditional: if AWS-2 ships monitoring; under the §7 de-scope this item moves to the first post-CP-6 increment.)** The health job emits the complete feature-aware matched-PSI report, publishes the frozen bounded CloudWatch summary set, and both alarms are tested by forced breach; email received.
- [ ] 8. Budgets $10/$50 live (test-fired); `project=delu` tag on every resource; `make aws-audit` returns **clean** at arc close.
- [ ] 9. The **first full calendar month after the first billable SageMaker resource is created** is **≤ $5**; the finalized monthly bill is archived in `docs/aws-bills/`. (This observation runs during the 6–8-week arc; it does not require a new month after all engineering ends.)
...
**Recommended (D8):** G5 closes ~late-Jan/early-Feb 2027 exactly as ratified (the AWS arc does **not** touch the flagship's pre-G5 critical path — inserting it earlier would delay the deployed showcase, the worst possible trade). **Month 6 (≈ Feb–Mar):** M6 at a realistic ~10 h/wk alongside active applications → **CP-6 closes ~mid-March/early-April** under §7's 6–8-week estimate. **Months 7–8 (≈ Apr–May):** companion FM0–FM5 as specified in its v1.0 doc, unchanged in content; **G6 → ~late-May/early-June 2027.** B-Man3 (Kaggle account) shifts from "Month 5" to the Month-6→7 seam; a new **B-Man-AWS** (account + IAM + Budgets, **no SageMaker resource**) lands at AWS-1 day 1. Program envelope ~705 h → **~755–775 h**.
...
**Ratification cascade (fires only on "yes"):** (1) capstone v6.3 → v6.4 per §11; (2) progress.md — anchors (doc version, envelope dates, budget line), Track B position, Notes; (3) **stage map v5 → v6 rebuild becomes owed** — per the standing rule it runs only on your explicit request, which ratifying D8 constitutes; FM rows shift one month, M6/CP-6/B-Man-AWS rows added; (4) the companion doc itself needs **no edit** (its content is month-agnostic; scheduling lives in the orchestrator layer); (5) `orchestrator-role.md` envelope sentence — owner-edited, flagged here for you to amend.
...
| Custom health job → CloudWatch | "How do you monitor a model in production?" → *"For the champion's retained non-deterministic features, I compute feature-aware PSI against a season/hour-matched frozen reference; I also track feed quality, interval-width shift, and quantile sanity. A bounded summary reaches CloudWatch with alarms, while the full per-feature artifact stays inspectable. With no labels at refresh time I call this input/output health, not performance drift. I built it from primitives because SageMaker's managed monitor was unavailable to new customers, and the Processing job made every assumption explicit."* |
...
## 10. Risk register additions (v6.4 §11, continuing R-1…R-5)
...
## 11. Exact amendment map — capstone v6.3 → v6.4

| v6.4 location | Amendment |
|---|---|
...
**Non-capstone cascade:** progress.md (anchors, Track B, Notes — regenerates on ratification), stage map **v5 → v6** (**approving D8 is itself the explicit rebuild authorization; no second go/no-go**), `orchestrator-role.md` envelope sentence (owner edit), CV slot usage per CP-6 item 13.

---

## 12. Ratification checklist (what "yes" means)

Reply approving **D1–D6 + D8** (or vetoing specific items) ratifies this spec as the **v6.4 amendment**. (D7 is already ratified and live in capstone v6.3 — it is not on this ballot.) Partial ratification remains legitimate — e.g., D1–D6 with a different D8 sequencing. Adjudication is scheduled: **DEC-AWS at G5**, on the inputs recorded in progress.md (C8 JD cloud-requirement ratio; live funnel and interview signal; February capacity). On ratification the cascade in §8 fires, starting with the progress.md amendment and the **map v5 → v6 rebuild; approval of D8 is the explicit rebuild authorization, so there is no additional map go/no-go.** Until then this file is a flagged, unratified draft under the version-precedence rule.
